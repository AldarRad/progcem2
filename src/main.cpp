...
int main(int, char**){
    ...
    IPParser::parseFile("D:\\program\\labs2\\src\\ip_filter.tsv", IPList); 

    std::sort(IPList.rbegin(), IPList.rend()); 
   
    auto first_list = [](const IPStruct& ip)
    {
        std::cout << ip << "\n";
    };  

    auto second_list  = [](const IPStruct& ip)
    {
        if(ip[0] == 1) std::cout << ip << "\n";
    };  

    auto third_list = [](const IPStruct& ip)
    {
        if(ip[0] == 46 && ip[1] == 70) std::cout << ip << "\n";
    };   

    auto fourth_list = [](const IPStruct& ip)
    {
        if(ip[0] == 46 || ip[1] == 46 || ip[2] == 46 || ip[3] == 46) std::cout << ip << "\n";
    };  
